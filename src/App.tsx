...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-16">
      ...
      {/* Interactive JSON Memory DB Records Inspector panel (Answers "จะดูข้อมูลที่เก็บไว้ยังไง") */}
      <section className="max-w-7xl mx-auto px-4 mt-12" id="database-state-console">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden text-slate-100 relative">
          
          {/* Glowing pulse decoration */}
          <div className="absolute top-0 left-0 w-2 h-full bg-teal-500" />

          {/* Header Panel */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4.5 pl-6 bg-slate-950 border-b border-slate-850 gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider font-mono text-slate-200">
                  NEXUS DB INSPECTOR • ตรวจสอบฐานข้อมูลที่เก็บและสืบค้น (JSON Memory API)
                </h3>
              </div>
            </div>

            <button
              onClick={() => setShowLiveDbConsole(!showLiveDbConsole)}
              className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-teal-300 font-mono transition-colors border border-slate-700 cursor-pointer"
              id="toggle-db-console-btn"
            >
              {showLiveDbConsole ? "[- OFF] ปิดซ่อนแผง JSON" : "[+ ON] แผ่เปิดดูข้อมูลที่เก็บไว้"}
            </button>
          </div>

          {/* Expandable Database detail metrics */}
          <AnimatePresence>
            {showLiveDbConsole && (
              <motion.div
              ...
              >
                <div className="p-5 bg-slate-900/95 border-b border-slate-950">
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                    
                    {/* Stored strategy summary columns */}
                    <div className="lg:col-span-4 space-y-4">
                      
                      <div className="p-4 bg-slate-950/80 rounded-xl border border-slate-800/60">
                        <span className="text-[9px] uppercase font-bold tracking-widest text-slate-500 font-mono block">โครงสร้างสถาปัตยกรรม (How is data stored?)</span>
                        <p className="text-xs font-bold text-slate-100 mt-1">Full-Stack Live State Engine</p>
                        <p className="text-[11px] text-slate-400 mt-1.5 leading-relaxed">
                          ข้อมูลจัดเก็บและแก้ไขในหน่วยความจำเซิร์ฟเวอร์ Express (Non-Static API Server Side Storage) อัปเดตและประสานข้อมูลตรงกันระหว่าง 'หน้าบ้านลูกค้าและหลังบ้านแอดมิน' ทันที
                        </p>
                      </div>

                      <div className="p-4 bg-slate-950/80 rounded-xl border border-slate-800/60">
                        <span className="text-[9px] uppercase font-bold tracking-widest text-slate-500 font-mono block">API Endpoint (สำหรับสืบค้นและดึงข้อมูล)</span>
                        <p className="text-[11px] font-mono text-emerald-400 mt-1 hover:underline select-all select-none">
                          GET /api/workspaces
                        </p>
                        <p className="text-[10px] text-slate-400 mt-1.5">
                          เมื่อคุณกด "จอง" ในหน้าบ้าน หรือแอดมินแก้ไขราคาคราบในหน้าหลังบ้าน การเรียก API นี้จะสะท้อนความแปลเปลี่ยนไปแบบ Reactive ทันที!
                        </p>
                      </div>

                      <div className="p-4 bg-slate-950/80 rounded-xl border border-slate-800/60">
                        <span className="text-[9px] uppercase font-bold tracking-widest text-slate-500 font-mono block">สิทธิ์การจองไม่ซ้ำซ้อน (Concurrency Rule)</span>
                        <p className="text-[11px] text-slate-300 mt-1 font-semibold">หลังบ้านคุ้มครองป้องกันจองชนกัน</p>
                        <p className="text-[10px] text-slate-400 mt-1 leading-relaxed">
                          หากจองระหว่างที่ห้องถูกจองแล้ว หรือลบระหว่างใช้งาน API หลังบ้านจะส่งรหัสสกัดบล็อก 400 Bad Request ป้องกันอย่างปลอดภัยแบบไร้ข้อผิดพลาด
                        </p>
                      </div>

                    </div>

                    {/* LIVE JSON RAW VIEWER */}
                    <div className="lg:col-span-8 flex flex-col justify-between">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-[10px] text-slate-400 font-mono uppercase tracking-widest font-bold">ข้อมูลสภาวะ Record ล่าสุดที่เก็บไว้ (Stored Record Data)</span>
                        <button
                          onClick={() => fetchWorkspaces()}
                          className="text-[10px] text-slate-400 hover:text-white font-mono flex items-center gap-1.5 cursor-pointer bg-slate-800 px-2 py-1 rounded"
                        >
                          <RefreshCw className="w-3 h-3 text-emerald-400" /> สั่งกวาดข้อมูลใหม่
                        </button>
                      </div>

                      <div className="bg-slate-950 p-4.5 rounded-2xl border border-slate-805 overflow-x-auto max-h-[340px] overflow-y-auto">
                        <pre className="text-xs font-mono text-cyan-300 whitespace-pre scrollbar-thin select-all">
      ...
      {/* FOOTER DESIGNS */}
      <footer className="max-w-7xl mx-auto px-4 mt-16 pt-8 border-t border-slate-200 text-center text-slate-400 text-xs">
        <p className="font-semibold text-slate-500 font-sans text-[11px]">NEXUS CO-WORKING HUB • พัฒนาด้วยเทคโนโลยี Express Backend + Vite React Frontend เต็มรูปแบบ</p>
        <p className="mt-1 font-mono text-[9px] text-slate-400">ระบบรักษาความปลอดภัย ป้องกันการจองซ้ำ และรับส่งข้อมูลด้วย Dynamic JSON In-Memory Database</p>
      </footer>
